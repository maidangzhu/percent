      ...
      parameters: {
        ...
        properties: {
          should_create: {
            type: "boolean",
            description:
              "仅当新增消息让一个与用户本人、聊天对象或双方关系直接相关的待办/提醒明确成立时为 true。第三方经历转述、吐槽、评论、闲聊里提到时间或安排，都必须为 false。",
          },
      ...
      messages: [
        {
          role: "system",
          content:
            "你是待办识别助手。你只基于新增聊天判断是否要创建待办/提醒。规则要严格：只有新增消息让一个与「我」、聊天对象、或我和聊天对象之间的约定直接相关的未来事项明确成立，才创建。不要因为历史上下文里重复出现的旧内容重复创建。必须创建的例子：对方约我某时间见面/电话/上门；我承诺要发资料/处理事情；对方要求我在某时间回复；我确认了双方之间的预约。必须不创建的例子：朋友转述他自己的面试/复试/约会/工作安排；聊天中吐槽第三方老板、同事或公司；只是讨论别人遇到的事；纯闲聊、情绪表达、评论、疑问句，或只是提到日期时间但没有让我参与、等待、跟进或记住。若历史上下文里已经提出与我直接相关的事项，而本次新增消息是「好的」「可以」「明天」「那就这样」等确认，才结合上下文创建提醒。",
        },
        {
          role: "user",
          content: `当前时间：${occurredAt.toISOString()}\n聊天对象：${personName}\n\n最近上下文：\n${contextText}\n\n本次新增消息：\n${newText}\n\n请判断本次新增消息是否让一个与我直接相关的待办/提醒成立。典型应创建：维修师傅说明明天过来、对方约定某个时间来、我回复好的确认预约、双方约见面/电话/处理事情。典型不应创建：朋友说他被约周日复试、朋友吐槽老板/公司、我只是评论「怎么还周日面试」。title 要写成用户视角的提醒，例如「明天等维修师傅上门」。`,
        },
      ],
      tools,
